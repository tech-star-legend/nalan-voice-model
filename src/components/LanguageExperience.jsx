...
function pickVoice(language) {
  ...
  const wanted = language === "ta" ? /^ta/i : /^en/i;
  const matching = voices.filter((voice) => wanted.test(voice.lang));
  ...
  const female = matching.filter((voice) => femaleHints.test(voice.name));
  const male = matching.filter((voice) => maleHints.test(voice.name));
  const pool = Math.random() < 0.5 ? female : male;
  return (pool.length ? pool : matching)[Math.floor(Math.random() * (pool.length ? pool.length : matching.length))];
}
...
function speak(language) {
  return new Promise((resolve) => {
    ...
    utterance.rate = language === "ta" ? 0.92 : 0.9;
    utterance.pitch = 1;
...
function LanguageExperience() {
  ...
  const selectLanguage = async (nextLanguage) => {
    ...
    setPhase("voice");
    setSpeaking(true);
    const minimum = new Promise((resolve) => setTimeout(resolve, 5200));
    await Promise.all([speak(nextLanguage), minimum]);
    setSpeaking(false);
      ...
      {!showIntro && (
        <div className="fixed right-3 top-3 z-[100] md:right-5 md:top-5">
          <div className="flex items-center gap-1 rounded-full border border-green-200 bg-white/95 p-1 shadow-lg backdrop-blur-xl">
            ...
            <button type="button" onClick={() => changeLanguage(language === "en" ? "ta" : "en")} aria-label="Change language" className="min-w-9 rounded-full px-2.5 py-1.5 text-sm font-bold text-green-800 transition hover:bg-green-50">
              {language === "en" ? "E" : "த"}
            </button>
          </div>
        </div>
          ...
          <motion.div className="fixed inset-0 z-[200] flex items-center justify-center overflow-hidden bg-[#06150d] px-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <motion.div className="relative flex w-full max-w-md flex-col items-center text-center" initial={{ scale: 0.78, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.7 }}>
              <motion.div animate={{ y: [0, -6, 0], rotate: [-1, 1, -1] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} className="relative">
                ...
                <motion.div animate={{ y: [12, -45], opacity: [0, .6, 0], scale: [.7, 1.25] }} transition={{ duration: 2.2, repeat: Infinity }} className="absolute -top-4 left-1/2 -translate-x-1/2 text-white/60"><span className="block h-8 w-4 rounded-full bg-white/20 blur-md" /></motion.div>
              ...
              {chooser && (
                <motion.div className="mt-8 w-full" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
                  <p className="mb-4 text-lg font-semibold text-white">Choose your language</p>
